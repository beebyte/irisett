...
from irisett.notify import clicksend
...
def parse_settings(config: Any) -> Optional[Dict[str, Any]]:
    ...
    if not provider:
        log.msg(
            "No SMS provider specified, no sms notifications will be sent",
            "NOTIFICATIONS",
        )
        return None
...
async def send_alert_notification(
    settings: Dict[str, Any], recipients: Iterable[str], tmpl_args: Dict[str, Any]
):
